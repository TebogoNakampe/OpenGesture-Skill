    ...
    public partial class IRCExample : Form
    {
        ...
        private static Dictionary<string, Func<IRCExample, string, string, string, string>> RequestHandlers =
            new Dictionary<string, Func<IRCExample, string, string, string, string>>()
            {
                ...
                { "OpenGestureIntent",  ((thisObj, target, topic, requestbody) => thisObj.OpenGesture(target,topic,requestbody))}
            };
        ...
        private string OpenGesture(string target, string topic, string requestbody)
        {
            ...
            if (response == null)
            {
                ...
                string searchQueryValue = irc.GetSlotString(requestbody, "searchQuery");
                string dateValue = irc.GetSlotString(requestbody, "date");
                OnLogMessage(string.Format("OpenGestureIntent-> appLocationValue: {0}, appTypeValue: {1}, dateValue: {2}, searchQueryValue: {3}", appLocationValue, appTypeValue, dateValue, searchQueryValue), null, true);
        ...
        public void PerformOpenGesture(string appLocationValue, string appTypeValue, string dateValue, string searchQueryValue = "")
        {
            ...
            if (!string.IsNullOrEmpty(searchQueryValue))
            {
                queryBuilder.Append("search-ms:displayname=Searching for query%20" + searchQueryValue + "%20in%20" + appLocationValue);
                if (!fileTypeValue.Equals("everything"))
